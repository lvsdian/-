...
4. `blocked`：线程在**等待进入synchronized代码块**或者调用wait方法后重新等待进入synchronized代码块
5. `waiting`：无限期等待，不会被`CPU`分配时间，需要等待被其他线程显式唤醒。**调用`Object.wait`，`Thread.join`，`LockSupport.park`等方法后进入此状态**
...
- **happens-before(先行发生)**
  - 先行发生原则是JMM 中定义的两项操作之间的偏序关系。如果操作A先行发生于操作B，就说明在发生操作B之前，操作A的影响能被操作B观察到，“影响”包括内存中共享变量的值，发送消息、调用方法等。
  - **规则制定了在一些特殊情况下，不允许编译器、处理器对代码进行指令重排，必须保证代码的有序性。**
### volatile
...
- 如果线上机器突然宕机，线程池阻塞队列中的请求怎么办
  - 如果机器宕机，那么积压的任务会丢失
  - 所以在提交任务到线程池之前，要将任务持久化，每次完成任务时更新任务状态，系统故障重启后再执行还未执行的任务。
